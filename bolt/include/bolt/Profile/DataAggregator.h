...
private:
  ...
  /// Used for parsing specific cfggrind input files.
  struct CfgGrindNodeEntry {
    uint64_t Cfg;
    Location NodeStart;
    uint64_t Size;
    std::vector<Location> Instrs;
    std::unordered_map<Location, uint64_t> Calls;
    std::unordered_map<Location, uint64_t> Branches; 
    uint64_t Count;
    bool IsIndirect;
  };
  ...
  /// Parse cfggrind output node entry
  ErrorOr<CfgGrindNodeEntry> CfgGrindNodeEntry();
